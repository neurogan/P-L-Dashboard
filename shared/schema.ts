...
export const products = plDashboard.table("products", {
  id: serial("id").primaryKey(),
...
// ─── Weekly Metrics (unified: Amazon + Shopify + Faire) ───────────────────

export const weeklyMetrics = plDashboard.table("weekly_metrics", {
  id: serial("id").primaryKey(),
  sku: text("sku").notNull(),
  asin: text("asin"),
  ...
  feeSource: text("fee_source"), // "settlement" or "estimated"
  ...
  uniqueIndex("wm_sku_week_channel_idx").on(table.sku, table.weekStartDate, table.channel),
  index("wm_week_idx").on(table.weekStartDate),
  index("wm_channel_idx").on(table.channel),
]);

export const insertWeeklyMetricsSchema = createInsertSchema(weeklyMetrics).omit({ id: true });
...
export const insertCogsPeriodSchema = createInsertSchema(cogsPeriods).omit({ id: true });
export type InsertCogsPeriod = z.infer<typeof insertCogsPeriodSchema>;
export type CogsPeriod = typeof cogsPeriods.$inferSelect;

// ─── Ad Weekly Summary (per-ASIN Amazon ad data) ─────────────────────────

export const adWeeklySummary = plDashboard.table("ad_weekly_summary", {
  id: serial("id").primaryKey(),
...
export const insertAdWeeklySummarySchema = createInsertSchema(adWeeklySummary).omit({ id: true });
...
export const dashboardMeta = plDashboard.table("dashboard_meta", {
  id: serial("id").primaryKey(),
  key: text("key").notNull().unique(),
  value: text("value").notNull(),
});

export const insertDashboardMetaSchema = createInsertSchema(dashboardMeta).omit({ id: true });
